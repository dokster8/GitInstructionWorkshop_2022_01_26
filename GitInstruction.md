...
Мы выяснили, что репозиторию могут быть локальными (на нашей машине) и удаленными (в сети).
...
Наиболее удобный способ для работы с удаленными репозиториями - GitHub.

Поэтому надо зарегистрироваться на сайте [GitHub](https://github.com/)

### **Отправляем свой локальный репозиторий в GitHub, работаем в удаленном репозитории и переносим изменения из удаленного репозитория в свой локальный**

1) **Создаем локальный репозиторий при помощи команды `git init`**

Проводим необходимую работу (добавляем файлы, делаем коммиты)

2) **Создаем новый репозиторий в GitHub:**

* В парвом верхнем углу нажимаем на `+`, далее `new repository`

* Задаем навзвание для нашего нового удаленного репозитория заполнив окошко `Repository name`
